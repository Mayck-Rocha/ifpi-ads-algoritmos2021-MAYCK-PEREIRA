...
function main(){
    const data = prompt('Insira uma data no formato 00/00/0000: ')
    ...
    for(let i = 0; i < data.length; i++){
        ...
        }else if(mes_string.length !== 2){
            mes_string = mes_string + caractere
        }else{
            ano = ano + caractere
        }
    }

    let mes_numero = Number(mes_string)
    let mes_f = mes_fomatado(mes_numero)
